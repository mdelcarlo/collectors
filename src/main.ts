...
function setupIpcHandlers(mainWindow: BrowserWindow) {
  ...
  ipcMain.handle('upload-videos', async () => {
    ...
    if (!result.canceled && result.filePaths.length > 0) {
      ...
      // Get existing unpaired videos
      const existingUnpaired = store.get('unpairedVideos') as any[];
      const existingPaired = store.get('pairs') as any[];

      let updatedUnpaired = [...existingUnpaired, ...videos];
      store.set('unpairedVideos', updatedUnpaired);

      // Run automatic pairing algorithm
      const { pairs, unpaired } = await videoMatcher.matchVideos(updatedUnpaired);

      // Update store with new pairs and remaining unpaired videos
      store.set('pairs', pairs);
      store.set('unpairedVideos', unpaired);
      ...
      const videosWithMeta = await metaGenerator.generateBatch(videos);


      // Add new videos to unpaired list
      updatedUnpaired = [...existingUnpaired, ...videosWithMeta];
      store.set('unpairedVideos', [...existingUnpaired, ...videosWithMeta]);

      // Run automatic pairing algorithm
      const groupedVideos = await videoMatcher.matchVideos(updatedUnpaired);

      // Update store with new pairs and remaining unpaired videos
      store.set('pairs', groupedVideos.pairs);
      store.set('unpairedVideos', groupedVideos.unpaired);
      ...
      return { pairs: groupedVideos.pairs, unpaired: groupedVideos.unpaired };
    }
  ...
  ipcMain.handle('process-media', async (_, videoIds) => {
    try {
      ...
      const unpaired = store.get('unpairedVideos') as any[];
      const allVideos = [
        ...(pairs.flatMap(p => [p.video1, p.video2])).map(videos => ({...videos, status: 'processing'})),
        ...unpaired
      ]

      let updatedPairs = pairs.map(pair => ({
        ...pair,
        video1: { ...pair.video1, status: 'processing' },
        video2: { ...pair.video2, status: 'processing' }
      }));
